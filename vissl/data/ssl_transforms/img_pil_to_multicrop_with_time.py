...
class ImgPilToMultiCropWithTime(ClassyTransform):
    ...
    def __call__(self, video: torch.Tensor) -> List[torch.Tensor]:
        ...
        # fig.savefig("/data1/runs/dcv2_ir108-cm_100x100_8frames_k9_70k_nc_r2dplus1/crop_time_all.png", bbox_inches='tight')
        # plt.close()
    ...
    def _apply_temporal_crop(self, crops: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        #print(f"Temporal crop window length: {win_len}, overlap frames: {min_overlap_frames}–{max_overlap_frames}")
        ...
        #print(f"Adjusted overlap frames: {min_overlap_frames}–{max_overlap_frames}")

        # Random start for first view
        start1 = random.randint(0, T - win_len)
        #print(f"Temporal crop start for view 1: {start1}")
        ...
        #print(f"Allowed range for start2: {start2_min}–{start2_max}")
